...
| `POST /api/staff/visits/{visitId}/arcs`<br>`POST /api/staff/arcs/{arcId}/revisions`<br>`POST /api/staff/arcs/{arcId}/revisions/{revisionId}/share`<br>`GET /api/staff/arcs/{arcId}` | 폼과 완료 화면은 Visit Memory 와 같은 구조로 이미 서 있습니다. 그런데 **제품 variant UUID 를 구할 방법이 없어 생성 자체가 불가능합니다** — 아래 "막힌 것" 1·2 참고. Arc 흐름만 아직 로컬로 둡니다.                              |
...
| **구매 날짜**              | 안내가 `YYYY.MM.DD` 였습니다 | `YYYY-MM-DD` 로 고쳤습니다. 점으로 적으면 400 입니다.                                                                               |
...
- **구매 매장** — `GET /api/stores` 로 검색할 수 있는데, Arc 생성 자체가 막혀 있어 지금 붙여도
  보낼 데가 없습니다. Arc 가 풀릴 때 `useStoreSearch()` 를 그대로 재사용하면 됩니다.
- **구매 국가** — 서버가 검증을 하지 않습니다. `KR` `Korea` `대한민국` `REPUBLIC OF KOREA` 를
  다 받습니다. 무엇을 넣을지 **팀에서 정해야 합니다.** (`StoreSummary.countryCode` 가 `KR` 이라
  코드로 맞추는 편이 자연스럽습니다.)
- **제품** — 카탈로그 API 가 없습니다. Visit Memory 는 `productEngagements` 를 비워 보낼 수 있어
  이것 없이도 동작하지만, 직원이 고른 제품은 서버에 남지 않습니다. "막힌 것" 2 번 참고.
...
### 2. Arc 는 제품 UUID 없이는 만들 수 없는데, 제품을 구할 데가 없습니다 <sup>(치명)</sup>
...
(`00000000-0000-0000-0000-00000000000N`)도 전부 500 이라, 씨앗 데이터로도 얻을 수 없습니다.
**Arc 생성은 여기서 완전히 막힙니다.** 필요한 것 두 가지입니다.

- 매장의 제품 variant 를 검색·나열하는 API (`GET /api/stores` 와 같은 모양이면 됩니다).
  `/staff/record-form` 의 `구매 제품` `관심 제품` 칸이 이걸 씁니다.
...
### 2-2. 방문에 딸린 Arc / Visit Memory 를 되찾을 수 없습니다 <sup>(새로 찾음)</sup>
