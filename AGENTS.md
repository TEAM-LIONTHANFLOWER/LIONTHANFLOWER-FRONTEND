...
**에이전트는 개발 서버를 띄우거나 브라우저로 화면을 직접 확인하지 않습니다.**
`npm start` / `npm run web` 실행, 브라우저 자동화로 화면을 눌러 보는 일은 하지 않습니다.
이미 돌고 있는 개발 서버에 붙는 것도 마찬가지입니다 — 개발 서버는 사람이 띄워 쓰는 것이라,
포트를 물고 있거나 창을 건드리면 쓰던 사람이 방해받습니다.

에이전트가 확인하는 범위는 `typecheck` / `lint` / `format` 까지입니다.
움직임과 모양이 맞는지는 사람이 봅니다. 그 대신 **무엇을 어디서 봐야 하는지** 를 적어 줍니다 —
어느 화면(`/arc` 처럼)에서 무엇을 눌렀을 때 무엇이 달라지는지.
